...
import com.sinosoft.config.CasClientConfig;
...
public abstract class AbstractConfigurationFilter implements Filter {
	 ...
	 * returns that, otherwise it retrieves the ServletContext's initParameters and
	 * returns that value if any.
	 * <p>
	 ...
	 * @return the property value, following the above conventions. It will always
	 *         return the more specific value (i.e. filter vs. context).
	 */
	...
	protected final String getPropertyFromInitParams(final FilterConfig filterConfig, final String propertyName,
		...
		switch (propertyName) {
		case "casServerLoginUrl":
			value = CasClientConfig.getCasServerLoginUrl();
			break;
		case "serverName":
			value = CasClientConfig.getServerName();
			break;
		case "casServerUrlPrefix":
			value = CasClientConfig.getCasServerUrlPrefix();
			break;
		case "ignorePattern":
			value = CasClientConfig.getIgnorePattern();
			break;
		}
		
		if (value == null) {
			return getPropertyFromInitParamsByCas(filterConfig, propertyName, defaultValue);
		} 
		return value;
	}

	protected final String getPropertyFromInitParamsByCas(final FilterConfig filterConfig, final String propertyName,
			final String defaultValue) {
		...
		if (CommonUtils.isNotBlank(value3)) {
			logger.info("Property [{}] loaded from JNDI Filter Specific Property with value [{}]", propertyName,
					value3);
			return value3;
		}
